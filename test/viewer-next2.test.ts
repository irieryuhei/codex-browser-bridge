...
import { VIEWER_NEXT2_JS } from "../src/viewer-next2-assets.js";
import { renderViewerNext2Html } from "../src/viewer-next2-html.js";

const doms: JSDOM[] = [];

describe("renderViewerNext2Html", () => {
  afterEach(() => {
    vi.useRealTimers();
    while (doms.length > 0) {
  ...
  it("renders the next2 viewer shell with standalone assets", () => {
    const html = renderViewerNext2Html();
  ...
  it("updates the URL when a conversation is selected and supports filtering", () => {
    const viewer = bootViewer();
    const socket = viewer.socketAt(0);
    ...
    viewer.input("sessionFilterInput").dispatchEvent(new viewer.window.Event("input"));
    expect(viewer.sessionButtons().map((button) => button.dataset.sessionId)).toEqual(["sess_a"]);
  ...
  it("shows one pane at a time on mobile and returns to the list with the back button", () => {
    ...
    viewer.sessionButtons()[0]?.click();
    expect(viewer.panel("listPanel").hidden).toBe(false);
  ...
  it("clamps long final answers and expands them on demand", () => {
    const viewer = bootViewer();
    const socket = viewer.socketAt(0);
    ...
    const toggle = viewer.document.querySelector(".viewer2-expand") as HTMLButtonElement | null
    expect(toggle).not.toBeNull()
    if (!toggle) {
      throw new Error("Expected expand toggle")
    }
...
function bootViewer(
  ...
  const dom = new JSDOM(renderViewerNext2Html(), {
    url: options.url ?? "http://127.0.0.1:8765/viewer-next2/",
  ...
  dom.window.eval(VIEWER_NEXT2_JS)
